  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        if (index == context.read<DataProvider>().invitesForCompetition.length) {
          return Center(
              child: IconButton(icon: const Icon(Icons.add), onPressed: () => showInviteDialog(context)));
        }
        Invite invite = context.read<DataProvider>().invitesForCompetition[index];
        return Card(
          child: ListTile(
            title: Text(invite.name),
            subtitle: Text(invite.mail),
            trailing: IconButton(
              onPressed: () => context.read<DataProvider>().removeInvite(index), icon: const Icon(Icons.delete),
            ),
          ),
        );
      },
